...
- Makes `poetry install --no-root` fail even though we don't need those packages
...
- Store `poetry.lock` in repo so `poetry install --no-root` just uses it without resolving
- ✅ Index fully in config
- ✅ Reproducible
- ❌ Requires committing lock file (large)
- **Action:** Generate `poetry.lock`, commit it, use `poetry install --no-root` (no extras needed)
...
git add poetry.lock
# Now users just: poetry install --no-root -E ml -E vision
```
